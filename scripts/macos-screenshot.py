...
def capture_window(window_id, output_name, description=""):
    ...
    try:
        ...
        image = CGWindowListCreateImage(
            CGRectNull, kCGWindowListOptionIncludingWindow, window_id, kCGWindowImageDefault
        )
        ...
        result = subprocess.run(
            ["screencapture", "-x", "-o", f"-l{window_id}", str(output_path)], capture_output=True
        )
        ...
        subprocess.run(
            ["sips", "--resampleWidth", "800", str(output_path), "--out", str(web_path)],
            capture_output=True,
        )
...
def activate_electron():
    ...
    subprocess.run(
        ["osascript", "-e", 'tell application "Electron" to activate'], capture_output=True
    )
    time.sleep(0.5)
...
def click_relative(window, x_ratio, y_ratio):
    ...
    if result.returncode == 0:
        ...
        subprocess.run(
            ["osascript", "-e", f"do shell script \"printf '\\x1b[3;{y};{x}t'\""],
            capture_output=True,
        )
    time.sleep(0.3)
