...
const steps: Step[] = [
  {
    ...
    content:
      'A voice-controlled terminal that puts you in command. Speak naturally, execute precisely.',
    icon: <TerminalIcon />,
  },
  {
    title: 'Your command center',
    content:
      'The terminal is your workspace. Type commands directly, run scripts, or start Claude Code by typing "claude".',
    highlight: 'terminal',
    icon: <TerminalIcon />,
  },
  {
    ...
    content:
      'Press Alt+S anytime to start voice input. A floating panel will appear with your audio visualizer.',
    highlight: 'shortcut',
    icon: <KeyboardIcon />,
  },
  {
    ...
    content:
      'In Agent mode, your speech is converted into CLI commands. Say "list all files in the current directory" and get "ls -la".',
    icon: <ChatIcon />,
  },
  {
    title: 'Configure your setup',
    content:
      'Click the gear icon in the title bar to set your API key, choose your transcription model, and customize behavior.',
    highlight: 'settings',
    icon: <GearIcon />,
  },
  {
    ...
    content:
      "You're all set! Press Alt+S to start your first voice command, or just type in the terminal below.",
    icon: <RocketIcon />,
  },
];
...
const Onboarding: React.FC<OnboardingProps> = ({ onComplete }) => {
  ...
  return (
    <div
      className={`fixed inset-0 z-[100] flex items-center justify-center transition-opacity duration-200 ${isExiting ? 'opacity-0' : 'opacity-100'}`}
    >
      {/* Backdrop */}
      ...
      >
        ...
        <div className="p-8">
          {/* Icon */}
          <div className="flex justify-center mb-6 text-accent">{step.icon}</div>
          ...
          {/* Description */}
          <p className="text-crt-white/70 text-center leading-relaxed mb-8">{step.content}</p>
        ...
        <div className="px-8 py-4 bg-void-200/50 border-t border-void-300">
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            />
            <span className="text-xs text-crt-white/40 font-mono">Don't show this again</span>
          </label>
        </div>
      </div>
    </div>
